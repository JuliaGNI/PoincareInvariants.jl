...
# This could be included anywhere
...
function SecondPoincareInvariant{D, T}(Ω::ΩT, N::Integer) where {D, T, ΩT <: AbstractMatrix}
	if size(Ω) != (D, D)
		throw(ArgumentError("Ω must be a $D × $D AbstractMatrix, not $(size(Ω, 1)) × $(size(Ω, 2))"))
	end
	
	# In future we may have other implementations (like finite differences)
	# For now it's hard coded to use ChebyshevSetup though
	setup = ChebyshevSetup(D, T, Ω, N)
	SecondPoincareInvariant{D, T, ΩT, typeof(setup)}(Ω, setup)
end

function SecondPoincareInvariant{D, T}(Ω::ΩT, N::Integer, inplace::Bool) where {D, T, ΩT <: Callable}
	setup = ChebyshevSetup(D, T, Ω, N, inplace)
	SecondPoincareInvariant{D, T, ΩT, typeof(setup)}(Ω, setup)
end
...
function compute(pinv::SecondPoincareInvariant{D, T, ΩT, S}, phasepoints, p, t) where {D, T, ΩT, S <: ChebyshevSetup}
	setup = pinv.setup
	# Convert to basis of Chebyshev polynomials of the first kind
	C1coeffs = paduatransform!(setup.padua, phasepoints)
	
	# Get coefficients of derivatives in basis of Chebyshev polynomials of the second kind
	C2∂x, C2∂y = differentiate!(setup.diff, C1coeffs)
	
	# Calculate ∑ ∫ Ωᵢⱼ(z(x, y), p, t)⋅(∂z/∂x)ᵢ⋅(∂z/∂y)ⱼ dx dy
	integrate!(setup.int, pinv.Ω, C1coeffs, p, t, C2∂x, C2∂y)
end
...
function ConstantΩSetup(D, ::Type{T}, ::AbstractMatrix, paduanum) where T
...
) where {D, T, ΩT <: AbstractMatrix, S <: ConstantΩSetup}
...
## InPlaceNonCanonicalSetup ##

# Ω an inplace function Ω!(out, z, t)
struct InPlaceNonCanonicalSetup{
	T,  # phase space type
	PTP <: PaduaTransformPlan,
	DM <: AbstractMatrix,
	CM <: AbstractMatrix,
...
function NonCanonicalSetup(D, ::Type{T}, ::Callable, paduanum) where T
	checkpaduanum(paduanum)
	degree = Int(getdegree(paduanum))
...
function compute(
    pinv::SecondPoincareInvariant{D, T, ΩT, S},
    phasepoints::AbstractMatrix
) where {D, T, ΩT, S <: NonCanonicalSetup{true}}
	checkphasepoints(phasepoints, pinv.N, D)
