...
@testset "get_min_padua_num" begin
    @test PoincareInvariants.get_min_padua_num(20) == 21

    @test PoincareInvariants.get_min_padua_num(100) == 105
end

@testset "get_phase_points!" begin
    point_num = PoincareInvariants.get_min_padua_num(100)
    T = Float64
    N = 4
    ...
    pinv = PoincareInvariant2{4}(param_func, point_num)
    PoincareInvariants.get_phase_points!(param_func, pinv.phase_points, param_points, Val(4))

    @test pinv.phase_points[begin] == first.(param_func.(param_points))
    @test pinv.phase_points[end] == last.(param_func.(param_points))
end
...
end  # PoincareInvariants.jl testset
